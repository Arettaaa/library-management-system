            ...
            <div class="w-full overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @foreach ($collectionBooks as $colection)
                        <tr class="text-gray-700 dark:text-gray-400">
                            <td class="px-4 py-3">
                                <div class="flex items-center text-sm">
                                    ...
                                    <div>
                                        <a href="{{ route('book.show', $colection->id) }}" class="text-purple-600 hover:underline">
                                            <p class="font-semibold">{{ $colection->book->title }}</p>
                                            <p class="text-xs text-gray-600 dark:text-gray-400">
                                                {{ optional($colection->book->category)->name }}
                                            </p>
                                        </a>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm">{{ $colection->book->writer }}</td>
                            <td class="px-4 py-3 text-sm">{{ $colection->book->publisher }}</td>
                            <td class="px-4 py-3 text-sm">{{ $colection->book->pubyear }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    @if($colection->book->isBorrowed())
                                    ...
                                    @else
                                    <form action="{{ route('borrow.book', $colection->book->id) }}" method="POST">
                                        @csrf
